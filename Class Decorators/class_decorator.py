...
def _wrap_method_with_invariant_checking_proxy(cls, name, predicate):
    ...
    def invariant_checking_method_decorator(self, *args, **kwargs):
        ...
        if not predicate(self):
            raise RuntimeError("Class invariant {!r} violated for {!r}"
                               .format(predicate.__doc__, self))
        return result
...
def _wrap_property_with_invariant_checking_proxy(cls, name, predicate):
    ...
    invariant_checking_proxy = InvariantCheckingProxy(prop, predicate)
...
class InvariantCheckingProxy(PropertyDataDescriptor):
...
# Apply class decorators just like function decorators
